        ...
        for day in sign_lis:
            for _item, day_list in enumerate(s._cal.monthdayscalendar(year, month)):
                if day in day_list:
                    item = 'I00' + str(_item + 2)
                    column = '#' + str(day_list.index(day)+1)
                    s.drawline(item=item,column=column,widget=s._calendar)
    
    def drawline(s,item,column,widget):
        """绘制签到信息"""
        bbox = widget.bbox(item,column)
        if not bbox: # 日历尚不可见
            s.master.after(20, lambda : s.drawline(item = item, column = column, widget = widget))
            return
        x,y,width,height = bbox
        item_value = widget.item(item)['values']
        text = item_value[int(column[1]) - 1]
        text = '%02d' % text
        canvas = tk.Canvas(s._calendar,bg='red', borderwidth=0, highlightthickness=0)
        canvas.configure(width=width,height=height)
